...
std::string Network::get_info() {
    ...
    case GAME_RUNNING_STATUS:
        _tmp_state = NONE;
        return std::string("GAME_RUNNING:").append(_update[0]);
    case WIN_STATUS:
        _tmp_state = NONE;
        return _update[0];
    default:
...
//        std::cout << "new" << new_msg << "|" << _game_buffer_res.packet.msg << std::endl;
...
//        _update.insert(_update.begin(), new_msg);
        _core_state = GAME_RUNNING_STATUS;
        if (std::string(reinterpret_cast<const char *>(_game_buffer_res.packet.msg)).size() > 0) {
